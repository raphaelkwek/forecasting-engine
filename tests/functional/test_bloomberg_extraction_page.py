...
def test_the_toggle_defaults_to_multiple_files(page):
    assert page.toggle[0].value is True


def test_switching_the_toggle_off_accepts_a_single_file(page):
    page.toggle[0].set_value(False)
    result = page.run()
    assert result.file_uploader[0].accept_multiple_files is False
